...
public class Server {
	...
	/**
	 * A private thread to handle requests on a particular socket. The client terminates the dialogue by sending a single line containing only a period.
	 */
	private static class RequestHandler extends Thread {
		...
		/**
		 * Services this thread's client by first sending the client a welcome message then repeatedly reading strings and sending back the an appropriate response.
		 */
		@Override
		public void run()
		{
		...
		/**
		 * If key is null, tell client keys has still not been found Otherwise, return the key
		 */
		...
		/**
		 * If there are any incomplete keyspaces, return the first one. Otherwise, return the next highest keyspace.
		 *
		 ...
		 *
		 * @param clientKeySpace
		 *            keyspace formatted firstkey,lastkey that the client presumably did not complete
		 */
	...
	/**
	 * Run in an infinite loop listening on port 9898. When a connection is requested, it spawns a new thread to do the servicing and immediately returns to listening. The server keeps a unique client number for each client that connects.
	 */
